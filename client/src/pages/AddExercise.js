    ...
    <>
      <TableContainer component={Paper} className="table-container">
        ...
        >
          ...
          <TableBody>
            {rows
              .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) //IMP: Note .slice is used for pagination
              .map((row) => (
                <TableRow
                  key={row.id}
                ...
                >
                  <TableCell component="th" scope="row">
                    ...
                    <span>{row.primary_muscle_group}</span>
                  </TableCell>
                  <TableCell>
                    {" "}
                    <div className="image-container">
                      {" "}
                      <img
                        onClick={(_) => console.log(row.img)}
                        src={row.img.split(",").at(0)}
                      ></img>{" "}
                    </div>
                  </TableCell>
                </TableRow>
    ...
    return exerciseList.filter((e, index) => {
      if (
        e.primary_muscle_group
          .split(";")
          .includes(filterOption["target-muscle"]) &&
    ...
    <>
      <div className="search-filter">
        <label htmlFor="target-muscle">
          <p className="label-text">Target Muscle:</p>
          <Select
            className="filter-select"
            name="target-muscle"
            options={searchOptions.targetMuscleOptions}
            onChange={(e) =>
              setFilterOption({
                ...filterOption,
        ...
        <label htmlFor="equipment">
          ...
          <Select
            className="filter-select"
            name="equipment"
            options={searchOptions.targetEquipmentOptions}
            onChange={(e) =>
              setFilterOption({
                ...filterOption,
                [e.target.name]: e.target.value,
